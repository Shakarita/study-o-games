...
def login():
    ...
    if request.method == "POST":
        if not request.form.get("username"):
            return "#TODO username must not empty"
        elif not request.form.get("password"):
            return "#TODO password must not empty"
        con = sql.connect('./db/data.db')
...
def register():
    ...
    if request.method == "POST":
       ...
       # date = request.form.get('day','month','year')
        
        print(u_name, email, pw, c_pw, id_type, Fu_name)
    ...
    articles_title = menuArticle()
    return render_template("register.html", articles_title=articles_title, d = day,m = month, y = year)
